...
export default function App() {
  ...
  const [trackValues, setTrackValues] = useState(initialValues())
  ...
  useEffect(() => {
    ...
    if(editing == null){
      setTrackValues(initialValues())
    } else{
      const {fname, lname, bio} = members.find(mem => mem.id == editing)
      setTrackValues({fname, lname, bio})
    }
  }, [editing])
  ...
  const onChange = evt => {
    ...
    setTrackValues(preValues => ({...preValues, [id]: value}))
  }
  const edit = id => {
  ...
  const submitNewMember = () => {
    ...
    const {fname, lname, bio} = trackValues
